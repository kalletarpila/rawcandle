...
from dev_tools.run_report_canonical_v2_rolling2_markdown import main
...
def _insert_window_row(
    ...
    *,
    ...
    row = {
        ...
        "latest_bearish_relevance_class": classification_state or "BEARISH_TOKEN",
...
def test_cli_writes_markdown_to_stdout(tmp_path: Path):
    ...
    assert exit_code == 0
    assert "# Datacenter Rolling2 Canonical V2 Report" in stdout
    assert "NVDA" in stdout
    assert "SELL_PRESSURE_X" in stdout
    assert stderr == ""
...
def test_cli_returns_error_for_invalid_db_path():
    exit_code, stdout, stderr = _run_cli(
        [
            "--db",
            "/tmp/does-not-exist-report-canonical-v2-rolling2.db",
            "--signal-date",
            "2026-05-30",
            "--taxonomy-version",
            "DC_TAXONOMY_FULL_V1",
