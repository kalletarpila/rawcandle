...
from dev_tools.run_report_canonical_v2_rolling30_markdown import main
...
def test_cli_writes_markdown_to_stdout(tmp_path: Path):
    ...
    assert exit_code == 0
    assert "# Datacenter Rolling30 Canonical V2 Report" in stdout
    assert "AMD" in stdout
    assert "BUY_STATE_X" in stdout
    assert "EXIT_STATE_X" in stdout
    assert stderr == ""
...
def test_cli_can_write_markdown_to_output_file(tmp_path: Path):
    ...
    assert "BUY_STATE_X" in output_path.read_text(encoding="utf-8")
    assert "EXIT_STATE_X" in output_path.read_text(encoding="utf-8")
...
def test_cli_returns_error_for_invalid_db_path():
    exit_code, stdout, stderr = _run_cli(
        [
            "--db",
            "/tmp/does-not-exist-report-canonical-v2-rolling30.db",
            "--signal-date",
            "2026-05-30",
            "--taxonomy-version",
            "DC_TAXONOMY_FULL_V1",
