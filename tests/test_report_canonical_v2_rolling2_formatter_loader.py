...
def _insert_window_row(
    ...
    *,
    ...
    window_watchlist_status: str = "BREAKOUT_CANDIDATE",
    breakout_days: int = 0,
    pullback_days: int = 0,
    ...
    distance_to_ema20_pct: float | None = 3.0,
) -> None:
    row = {
...
def _insert_classification_row(
    ...
    *,
    ...
    ticker: str,
    market: str | None = "usa",
    classification_state: str = "ELEVATED_SELL_PRESSURE",
    primary_reason: str = "EXIT_RISK_STACK",
    risk_reason: str = "HIGH_EXIT_RISK",
    next_action: str = "REDUCE_OR_EXIT",
) -> None:
    conn.execute(
        """
...
def test_loader_reads_canonical_rolling2_data():
    conn = _connect()
    _insert_run(conn, run_id="run-1", created_at_utc="2026-05-30T00:00:00Z")
    _insert_group_row(conn, run_id="run-1", group_type="layer", group_name="Infrastructure", pct_above_ema20=62.5, pct_above_ma10=71.0)
    _insert_group_row(conn, run_id="run-1", group_type="subindustry", group_name="Semis", pct_above_ema20=58.0, pct_above_ma10=66.0)
    _insert_window_row(conn, run_id="run-1", ticker="NVDA", primary_layer="Infrastructure", primary_subindustry="Semis", breakout_days=2)
    _insert_classification_row(conn, run_id="run-1", ticker="NVDA")
    conn.commit()

    data = load_rolling2_canonical_formatter_data_v2(
        conn,
        signal_date="2026-05-30",
        taxonomy_version="DC_TAXONOMY_FULL_V1",
        market="usa",
...
def test_run_selection_behavior():
    ...
    latest = load_rolling2_canonical_formatter_data_v2(
        conn,
        signal_date="2026-05-30",
        taxonomy_version="DC_TAXONOMY_FULL_V1",
        market="usa",
    )
    explicit = load_rolling2_canonical_formatter_data_v2(
        conn,
        signal_date="2026-05-30",
        taxonomy_version="DC_TAXONOMY_FULL_V1",
        market="usa",
...
def test_market_filtering():
    ...
    assert [row["ticker"] for row in data["rolling2_sell_pressure_rows"]] == ["NVDA"]
    assert [row["group_name"] for row in data["group_rows"]] == ["Infrastructure"]


def test_rolling2_classification_join_uses_stored_classification():
    conn = _connect()
    _insert_run(conn, run_id="run-1", created_at_utc="2026-05-30T00:00:00Z")
    _insert_window_row(
        conn,
    ...
    _insert_classification_row(
        ...
        run_id="run-1",
        ticker="NVDA",
        classification_state="ELEVATED_SELL_PRESSURE",
        primary_reason="EXIT_RISK_STACK",
        risk_reason="HIGH_EXIT_RISK",
    ...
    row = data["rolling2_sell_pressure_rows"][0]
    assert row["ticker"] == "NVDA"
    assert row["classification_state"] == "ELEVATED_SELL_PRESSURE"
    assert row["primary_reason"] == "EXIT_RISK_STACK"
    assert row["risk_reason"] == "HIGH_EXIT_RISK"
    assert row["next_action"] == "REDUCE_OR_EXIT"
...
def test_watchlist_rows_use_stored_status():
    ...
    assert [
        ...
        ("AMD", "PULLBACK_CANDIDATE", "GROUP_RISK"),
        ("NVDA", "HIGH_EXIT_RISK", "HIGH_EXIT_RISK"),
    ]
...
def test_taxonomy_listing_rows_are_deterministic():
    ...
    _insert_window_row(conn, run_id="run-1", ticker="AAA", primary_layer="ALayer", primary_subindustry="ASub", distance_to_ema20_pct=1.5)
    conn.commit()
    ...
    assert rows[0]["pct_above_ema20"] is None
    assert rows[2]["distance_to_ema20_pct"] == 1.5
    assert "pct_above_ema20" not in rows[2]
...
def test_section_counts_are_computed_from_canonical_rows():
    ...
    _insert_classification_row(conn, run_id="run-1", ticker="AAA", classification_state="ELEVATED_SELL_PRESSURE")
    _insert_classification_row(conn, run_id="run-1", ticker="BBB", classification_state="DEFENSIVE_MONITOR")
    conn.commit()
    ...
    assert counts["rolling2_classification_state_counts"] == {
        "DEFENSIVE_MONITOR": 1,
        "ELEVATED_SELL_PRESSURE": 1,
    }
