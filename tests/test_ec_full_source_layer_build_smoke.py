    ...
    try:
        ...
        conn.execute(
            ...
            INSERT INTO dc_ticker_swing_signal_daily VALUES (
                ...
                'DC_SWING_SIGNAL_V1','TICKER_RUN_20260605','2026-06-07T06:00:00Z',NULL,1,'FRESH','UP','epoch-1',
...
def test_full_source_layer_build_runs_all_loaders_into_single_temp_db(tmp_path) -> None:
    ...
    taxonomy_summary = load_datacenter_taxonomy_to_ec_sidecar(
        db_path=str(target_db),
        taxonomy_csv_path=str(taxonomy_csv),
        taxonomy_version_code="DC_TAXONOMY_FULL_V1",
    )
    watchlist_summary = load_datacenter_watchlist_to_ec_sidecar(
        db_path=str(target_db),
        watchlist_path=str(watchlist_txt),
    )
    ticker_summary = load_ec_ticker_signal_daily_from_dc(
        source_db_path=str(source_db),
        target_db_path=str(target_db),
        signal_date="2026-06-05",
    )
    group_summary = load_ec_group_signal_daily_from_dc(
        source_db_path=str(source_db),
        target_db_path=str(target_db),
        signal_date="2026-06-05",
    )
    synth_summary = load_ec_group_synthetic_ohlc_daily_from_dc(
        source_db_path=str(source_db),
        target_db_path=str(target_db),
        signal_date="2026-06-05",
    )
    index_summary = load_ec_group_index_daily_from_dc(
        source_db_path=str(source_db),
        target_db_path=str(target_db),
        signal_date="2026-06-05",
    )
    watermark_summary = load_ec_pipeline_watermark_from_dc(
        source_db_path=str(source_db),
        target_db_path=str(target_db),
    )

    with _connect(str(target_db)) as conn:
        assert conn.execute("SELECT COUNT(*) FROM ec_ticker_signal_daily").fetchone()[0] == 1
        ...
        assert conn.execute("SELECT COUNT(*) FROM ec_membership m JOIN ec_entity e ON e.entity_id=m.child_entity_id WHERE e.entity_code='CRGY'").fetchone()[0] == 0
        assert conn.execute("SELECT COUNT(*) FROM ec_ticker_signal_daily WHERE ticker='CRGY'").fetchone()[0] == 0
        assert conn.execute("SELECT COUNT(*) FROM ec_ticker_signal_daily WHERE source_run_id IS NULL OR source_pk_json IS NULL OR source_row_hash IS NULL").fetchone()[0] == 0
        assert conn.execute("SELECT COUNT(*) FROM ec_group_signal_daily WHERE source_run_id IS NULL OR source_pk_json IS NULL OR source_row_hash IS NULL").fetchone()[0] == 0
        assert conn.execute("SELECT COUNT(*) FROM ec_group_synthetic_ohlc_daily WHERE source_run_id IS NULL OR source_pk_json IS NULL OR source_row_hash IS NULL").fetchone()[0] == 0
        assert conn.execute("SELECT COUNT(*) FROM ec_group_index_daily WHERE source_run_id IS NULL OR source_pk_json IS NULL OR source_row_hash IS NULL").fetchone()[0] == 0

    assert taxonomy_summary["ticker_count"] == 1
    assert watchlist_summary["loaded_member_count"] == 2
    assert watchlist_summary["created_watchlist_only_entity_count"] == 1
    assert watchlist_summary["watchlist_only_tickers"] == ["CRGY"]
    assert ticker_summary["status"] == "OK_WITH_WARNINGS"
    assert group_summary["status"] == "OK_WITH_WARNINGS"
    assert synth_summary["status"] == "OK_WITH_WARNINGS"
    assert index_summary["status"] == "OK_WITH_WARNINGS"
    assert watermark_summary["status"] == "OK_WITH_WARNINGS"
    assert watermark_summary["unknown_components"] == ["WEEKLY_REPORT"]
