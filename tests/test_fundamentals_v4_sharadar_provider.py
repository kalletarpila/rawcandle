...
def test_provider_observation_stays_separate_from_canonical_model() -> None:
    client = SharadarClient(api_key="key", opener=FakeOpener([]))
    records = client.raw_observations(
        "fundamentals",
        [{"ticker": "AAPL", "dimension": "ARQ", "reportperiod": "2025-12-27", "fiscalperiod": "2026-Q1", "sharesbas": 1}],
    )
    assert len(records) == 1
    assert isinstance(records[0], ProviderObservation)
    assert records[0].provider == "SHARADAR"
    assert records[0].native_table == "fundamentals"
    assert records[0].ticker == "AAPL"
    assert records[0].observed_period_end == "2025-12-27"
    assert records[0].provider_fiscal_label == "2026-Q1"
    assert records[0].fields["sharesbas"] == 1
    assert "shares_outstanding" not in records[0].fields


def test_provider_name_contract() -> None:
    client = SharadarClient(api_key="key", opener=FakeOpener([]))
    assert client.provider_name == "SHARADAR"


def test_explicit_q4_identity_preserved() -> None:
    row = {"ticker": "AAPL", "dimension": "ARQ", "reportperiod": "2025-09-27", "fiscalperiod": "2025-Q4"}
    observation = SharadarClient(api_key="key", opener=FakeOpener([])).raw_observations("fundamentals", [row])[0]
    assert observation.provider_fiscal_label == "2025-Q4"
    assert observation.observed_period_end == "2025-09-27"
...
def test_provider_module_has_no_swingmaster_runtime_dependency() -> None:
    import rawcandle.fundamentals.providers.sharadar as sharadar

    assert "rawcandle/fundamentals/providers/sharadar.py" in sharadar.__file__
