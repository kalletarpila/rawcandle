...
from dev_tools.run_report_canonical_v2_rolling30_csv import main
...
def _insert_window_row(
    ...
    *,
    ...
    market: str | None = None,
    current_watchlist_status: str = "CURRENT_BUY_X",
    window_watchlist_status: str = "WINDOW_BUY_X",
    latest_exit_reason: str | None = "EXIT_REASON_X",
) -> None:
    row = {
...
def _insert_buy_classification_row(
    ...
    *,
    ...
    ticker: str,
    market: str | None = None,
    classification_state: str = "BUY_STATE_X",
    primary_reason: str = "BUY_PRIMARY, X",
    blocking_reason: str | None = "BUY_BLOCK_X",
) -> None:
    conn.execute(
        """
...
def _insert_exit_classification_row(
    ...
    *,
    ...
    ticker: str,
    market: str | None = None,
    classification_state: str = "EXIT_STATE_X",
    primary_reason: str = "EXIT_PRIMARY_X",
    risk_reason: str | None = "EXIT_RISK_X",
) -> None:
    conn.execute(
        """
        INSERT INTO dc_report_classification_v2 (
...
def test_cli_writes_csv_to_stdout(tmp_path: Path):
    ...
    with _create_db(db_path) as conn:
        ...
        _insert_buy_classification_row(conn, run_id="run-1", ticker="AMD", classification_state="BUY_STATE_X")
        _insert_exit_classification_row(conn, run_id="run-1", ticker="AMD", classification_state="EXIT_STATE_X")
        conn.commit()
    ...
    assert any(row["classification_state"] == "BUY_STATE_X" for row in rows)
    assert any(row["classification_state"] == "EXIT_STATE_X" for row in rows)
    assert stderr == ""
...
def test_cli_can_write_csv_to_output_file(tmp_path: Path):
    ...
    with _create_db(db_path) as conn:
        ...
        _insert_buy_classification_row(conn, run_id="run-1", ticker="AMD")
        _insert_exit_classification_row(conn, run_id="run-1", ticker="AMD")
        conn.commit()
...
def test_cli_respects_explicit_run_id(tmp_path: Path):
    ...
    with _create_db(db_path) as conn:
        ...
        _insert_group_row(
            ...
            group_name="SemisA",
            parent_group_type="layer",
            parent_group_name="InfraA",
        )
        _insert_window_row(conn, run_id="run-a", ticker="AMD", primary_layer="InfraA", primary_subindustry="SemisA")
        _insert_buy_classification_row(conn, run_id="run-a", ticker="AMD", classification_state="BUY_A")
        _insert_exit_classification_row(conn, run_id="run-a", ticker="AMD", classification_state="EXIT_A")
        ...
        _insert_group_row(
            ...
            group_name="SemisB",
            parent_group_type="layer",
            parent_group_name="InfraB",
        )
        _insert_window_row(conn, run_id="run-b", ticker="NVDA", primary_layer="InfraB", primary_subindustry="SemisB")
        _insert_buy_classification_row(conn, run_id="run-b", ticker="NVDA", classification_state="BUY_B")
        _insert_exit_classification_row(conn, run_id="run-b", ticker="NVDA", classification_state="EXIT_B")
        conn.commit()
    ...
    rows = _parse_csv(stdout)
    metadata_rows = [row for row in rows if row["section"] == "metadata"]

    assert exit_code == 0
    assert any(row["key"] == "selected_run_id" and row["value"] == "run-a" for row in metadata_rows)
    assert any(row["ticker"] == "AMD" for row in rows)
...
def test_cli_applies_market_filter(tmp_path: Path):
    ...
    with _create_db(db_path) as conn:
        ...
        _insert_group_row(
            ...
            run_id="omxh-run",
            group_type="subindustry",
            group_name="NordicSemis",
            parent_group_type="layer",
            parent_group_name="NordicInfra",
            market="omxh",
        )
        ...
        _insert_buy_classification_row(conn, run_id="usa-run", ticker="AMD", market="usa")
        _insert_buy_classification_row(conn, run_id="omxh-run", ticker="NOKIA", market="omxh")
        _insert_exit_classification_row(conn, run_id="usa-run", ticker="AMD", market="usa")
...
def test_cli_returns_error_for_invalid_db_path():
    exit_code, stdout, stderr = _run_cli(
        [
            "--db",
            "/tmp/does-not-exist-report-canonical-v2-rolling30.csv.db",
            "--signal-date",
            "2026-05-30",
            "--taxonomy-version",
            "DC_TAXONOMY_FULL_V1",
...
def test_cli_csv_header_and_row_semantics_are_correct(tmp_path: Path):
    db_path = tmp_path / "analysis.db"
    with _create_db(db_path) as conn:
        _insert_run(conn, run_id="run-1", created_at_utc="2026-05-30T00:00:00Z")
        _insert_group_row(conn, run_id="run-1", group_type="layer", group_name="Infrastructure")
    ...
    assert "next_action" not in first_line
    assert buy_row["primary_reason"] == "BUY_PRIMARY, X"
    assert buy_row["blocking_reason"] == "BUY_BLOCK_X"
    assert buy_row["risk_reason"] == ""
    assert exit_row["primary_reason"] == "EXIT_PRIMARY_X"
    assert exit_row["risk_reason"] == "EXIT_RISK_X"
    assert exit_row["blocking_reason"] == ""
    assert buy_row["latest_exit_reason"] == ""
    assert buy_row["primary_subindustry"] == "Semis, Accelerators"
    assert stderr == ""
