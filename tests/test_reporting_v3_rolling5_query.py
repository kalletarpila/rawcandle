...
def _insert_classification(
    ...
    *,
    ...
    conn.execute(
        ...
        ) VALUES (?, ?, ?, ?, ?, ?, 'rolling5_pullback', ?, ?, ?, ?, ?, 'rolling5_query_test', 'V1', 'CLASS_SOURCE', 'OK')
        """,
        (
            RUN_ID,
            ecosystem_id,
...
def _insert_relevance(conn: sqlite3.Connection, signal_observation_id: int) -> None:
    conn.execute(
        ...
        ) VALUES (?, 'CONTEXTUAL', 1.0, 'rolling5 freshness only')
        """,
...
def _build_fixture_db(db_path: str) -> None:
    ...
    try:
        ...
        _insert_snapshot(conn, ecosystem_id=ecosystem_id, taxonomy_version_id=taxonomy_version_id, entity_id=subindustry_id, timing_state="TRIM_WATCH", trend_state="UP", summary_state="MIXED", freshness_status="AGING", quality_status="WARN")
        _insert_snapshot(conn, ecosystem_id=ecosystem_id, taxonomy_version_id=taxonomy_version_id, entity_id=nvda_id, trend_state="UP", summary_state="LEADER", classification_state="WRONG_FROM_SNAPSHOT", quality_status="OK")
        _insert_snapshot(conn, ecosystem_id=ecosystem_id, taxonomy_version_id=taxonomy_version_id, entity_id=crgy_id, trend_state="DOWN", summary_state="WEAK", classification_state="WRONG_FROM_SNAPSHOT", quality_status="WARN")
        ...
        for signal_date, metric_name, metric_value_num in (
            ("2026-05-26", "pct_above_ema20", 58.0),
            ("2026-05-29", "pct_above_ema20", 62.5),
            ("2026-05-26", "return_5d", -2.0),
            ("2026-05-29", "return_5d", 1.0),
            ("2026-05-29", "weakness_breadth", 30.0),
        ):
            _insert_metric_num(
                conn,
                ecosystem_id=ecosystem_id,
        ...
        for signal_date, metric_name, metric_value_text in (
            ("2026-05-26", "group_overheat_risk_level", "MEDIUM"),
        ...
        _insert_event(conn, ecosystem_id=ecosystem_id, taxonomy_version_id=taxonomy_version_id, entity_id=nvda_id, event_date="2026-05-28", event_type="BOS", event_key="nvda-bos")
        _insert_event(conn, ecosystem_id=ecosystem_id, taxonomy_version_id=taxonomy_version_id, entity_id=subindustry_id, event_date="2026-05-26", event_type="STRUCTURE_CHANGE", event_key="semis-structure")
...
def test_query_requires_existing_run_id(tmp_path) -> None:
    db_path = tmp_path / "rolling5_query_missing_run.db"
    _build_fixture_db(str(db_path))

    with pytest.raises(ValueError, match="Missing eco_report_run"):
        build_rolling5_report_query_data(str(db_path), "missing-run")


def test_query_returns_rolling5_structured_data_from_eco_facts(tmp_path) -> None:
    db_path = tmp_path / "rolling5_query.db"
    _build_fixture_db(str(db_path))
    ...
    assert data.ecosystem_window_change[0]["change"] == 4.5
    assert data.ecosystem_window_change[1]["change"] == 3.0
    assert data.ecosystem_window_change[2]["change"] == 0.0
    assert data.ecosystem_window_change[3]["first_value"] == "MEDIUM"
    assert data.ecosystem_window_change[3]["last_value"] == "LOW"
    ...
    assert len(data.rolling5_pullback_classifications) == 1
    assert data.rolling5_pullback_classifications[0]["ticker"] == "NVDA"
    assert data.rolling5_pullback_classifications[0]["classification_state"] == "PULLBACK_CANDIDATE"
    assert data.rolling5_pullback_classifications[0]["blocking_reason"] == "EXIT_RISK_DAYS_WITHOUT_HIGH_SEVERITY"
    assert data.rolling5_pullback_classifications[0]["next_action"] == "REVIEW_FOR_DAILY_TRIGGER"

    assert data.ticker_snapshots[1]["classification_state"] == "WRONG_FROM_SNAPSHOT"
    assert data.rolling5_pullback_classifications[0]["classification_state"] != data.ticker_snapshots[1]["classification_state"]

    assert data.ticker_metrics["NVDA"]["breakout_days"] == 1.0
    ...
    assert data.watchlist_summary["rows"][0]["breakout_days"] == 1.0
    assert data.watchlist_summary["rows"][0]["pullback_days"] == 2.0
    assert data.watchlist_summary["rows"][0]["exit_risk_days"] == 3.0
    ...
    assert data.metadata["rolling5_classification_source"] == "eco_classification_decision"
    assert data.metadata["rolling5_snapshot_classification_source_used"] is False
    assert data.metadata["rolling5_event_window_mode"] == "event_date_range_within_5d_window"
    ...
    assert data.metadata["rolling5_event_window_mode"] != "event_date_range_within_30d_window"
