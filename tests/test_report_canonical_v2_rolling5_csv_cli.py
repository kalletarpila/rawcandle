...
from dev_tools.run_report_canonical_v2_rolling5_csv import main
...
def _insert_window_row(
    ...
    *,
    ...
    latest_exit_reason: str | None = "STRUCTURAL_WARNING",
) -> None:
    row = {
...
def _insert_classification_row(
    ...
    *,
    ...
    primary_reason: str = "PRIMARY_PULLBACK, X",
) -> None:
    conn.execute(
        """
...
def test_cli_writes_csv_to_stdout(tmp_path: Path):
    ...
    with _create_db(db_path) as conn:
        ...
        _insert_window_row(conn, run_id="run-1", ticker="AMD", primary_layer="Infrastructure", primary_subindustry="Semis, Accelerators")
    ...
    assert any(row["classification_state"] == "PULLBACK_STATE_X" for row in rows)
    assert stderr == ""
...
def test_cli_filters_by_market(tmp_path: Path):
    db_path = tmp_path / "analysis.db"
    with _create_db(db_path) as conn:
        _insert_run(conn, run_id="run-usa", created_at_utc="2026-05-30T00:00:00Z", market="usa")
        _insert_group_row(conn, run_id="run-usa", group_type="layer", group_name="Infrastructure USA", market="usa")
        _insert_group_row(
            conn,
            run_id="run-usa",
            group_type="subindustry",
            group_name="Semis, Accelerators USA",
            parent_group_type="layer",
            parent_group_name="Infrastructure USA",
            market="usa",
        )
        _insert_window_row(conn, run_id="run-usa", ticker="AMD", primary_layer="Infrastructure USA", primary_subindustry="Semis, Accelerators USA", market="usa")
        _insert_classification_row(conn, run_id="run-usa", ticker="AMD", market="usa")

        _insert_run(conn, run_id="run-omxh", created_at_utc="2026-05-30T00:00:00Z", market="omxh")
        _insert_group_row(conn, run_id="run-omxh", group_type="layer", group_name="Infrastructure OMXH", market="omxh")
        _insert_group_row(
            conn,
            run_id="run-omxh",
            group_type="subindustry",
            group_name="Semis, Accelerators OMXH",
            parent_group_type="layer",
            parent_group_name="Infrastructure OMXH",
            market="omxh",
        )
        _insert_window_row(conn, run_id="run-omxh", ticker="NOKIA", primary_layer="Infrastructure OMXH", primary_subindustry="Semis, Accelerators OMXH", market="omxh")
        _insert_classification_row(conn, run_id="run-omxh", ticker="NOKIA", market="omxh", classification_state="STATE_OMXH")
        conn.commit()
...
def test_cli_does_not_require_source_tables(tmp_path: Path):
    db_path = tmp_path / "analysis.db"
    with _create_db(db_path) as conn:
        _insert_run(conn, run_id="run-1", created_at_utc="2026-05-30T00:00:00Z")
        _insert_group_row(conn, run_id="run-1", group_type="layer", group_name="Infrastructure")
    ...
    assert exit_code == 0
    assert stderr == ""
    assert any(row["section"] == "rolling5_pullback_rows" for row in rows)


def test_cli_invalid_db_path_returns_error():
    exit_code, stdout, stderr = _run_cli(
        [
            "--db",
            "/does/not/exist.db",
...
def test_cli_missing_required_args_returns_usage_error():
    exit_code, stdout, stderr = _run_cli(["--db", "analysis.db"])
...
def test_cli_csv_header_and_csv_specific_sanity(tmp_path: Path):
    db_path = tmp_path / "analysis.db"
    with _create_db(db_path) as conn:
        _insert_run(conn, run_id="run-1", created_at_utc="2026-05-30T00:00:00Z")
        _insert_group_row(conn, run_id="run-1", group_type="layer", group_name="Infrastructure")
        ...
        _insert_classification_row(conn, run_id="run-1", ticker="AMD", primary_reason="PRIMARY_PULLBACK, X")
        conn.commit()
    ...
    assert pullback_row["primary_reason"] == "PRIMARY_PULLBACK, X"
    assert pullback_row["latest_exit_reason"] == ""
