...
from .contract import DIAGNOSTIC_MODEL_VERSION, model_fingerprint
...
MODEL_CONTRACT = {
    ...
    "definitions": {
        "ABRUPT_FUNDAMENTAL_SHIFT": {
            "formula": "max(abs(delta_revenue)/R,abs(delta_operating_income)/R)",
        ...
        "VALUATION_YIELD_OUTLIER": {
            ...
            "yields": ("operating_income_over_ev", "fcf_over_market_cap", "common_earnings_over_market_cap"),
            "operators": (">=", ">="),
            "thresholds": (0.25, 0.50),
            "required_status": "VALUATION_FULL",
        },
        "RECENT_MARGIN_DECELERATION_REVIEW": {
            "formula": "trajectory>=7 AND operating_margin_t-operating_margin_t_minus_1<=-0.02",
            "operators": (">=", "<="),
            "thresholds": (7.0, -0.02),
            "positive_revenue": True,
            "positive_operating_income_required": False,
        },
        "WORKING_CAPITAL_SHIFT_CANDIDATE": {
...
# This is the existing persisted 16-slot V2 layout, expressed natively.
EVIDENCE_FIELDS: dict[str, tuple[str, ...]] = {
    FLAG_NAMES[0]: ("current_revenue", "prior_revenue", "current_operating_income", "prior_operating_income", "delta_revenue", "delta_operating_income", "revenue_scale", "revenue_shift_ratio", "operating_income_shift_ratio", "metric_value", "threshold"),
    ...
    FLAG_NAMES[4]: ("operating_income_yield", "fcf_yield", "earnings_yield", "available_yield_count", "median_yield", "maximum_yield", "median_threshold", "maximum_threshold"),
    FLAG_NAMES[5]: ("current_revenue", "prior_revenue", "current_operating_income", "prior_operating_income", "current_operating_margin", "prior_operating_margin", "signed_margin_change", "current_trajectory", "trajectory_threshold", "margin_change_threshold"),
...
BOOLEAN_FIELDS: dict[str, tuple[str, ...]] = {
    FLAG_NAMES[0]: ("revenue_trigger", "operating_income_trigger"),
...
class DiagnosticInput:
    ...
    canonical_fiscal_chain_consecutive: bool | None = None
...
def _abrupt(data: DiagnosticInput) -> FlagEvaluation:
    ...
    if gate:
        return gate
    values = _comparison_values(data, ("revenue", "operating_income"))
    ...
    delta_operating_income = float(data.current.operating_income) - float(data.prior.operating_income)
    revenue_ratio, operating_income_ratio = abs(delta_revenue) / scale, abs(delta_operating_income) / scale
    revenue_trigger, operating_income_trigger = revenue_ratio >= 0.20, operating_income_ratio >= 0.20
    return _evaluated(data, flag, revenue_trigger or operating_income_trigger, "ABRUPT_SHIFT_THRESHOLD_MET" if revenue_trigger or operating_income_trigger else "ABRUPT_SHIFT_BELOW_THRESHOLD", _evidence(
        current_revenue=data.current.revenue, prior_revenue=data.prior.revenue,
        current_operating_income=data.current.operating_income, prior_operating_income=data.prior.operating_income,
        delta_revenue=delta_revenue, delta_operating_income=delta_operating_income, revenue_scale=scale,
        revenue_shift_ratio=revenue_ratio, operating_income_shift_ratio=operating_income_ratio,
        revenue_trigger=revenue_trigger, operating_income_trigger=operating_income_trigger,
        metric_value=max(revenue_ratio, operating_income_ratio), threshold=0.20, boundary_operator=">=",
    ))
...
def _valuation(data: DiagnosticInput) -> FlagEvaluation:
    ...
    yields = {"operating_income_yield": current.operating_income_yield, "fcf_yield": current.fcf_yield, "earnings_yield": current.earnings_yield}
    ...
    return _evaluated(data, flag, triggered, "VALUATION_YIELD_OUTLIER_THRESHOLD_MET" if triggered else "VALUATION_YIELDS_BELOW_THRESHOLDS", _evidence(
        operating_income_yield=current.operating_income_yield, fcf_yield=current.fcf_yield, earnings_yield=current.earnings_yield,
...
def _margin(data: DiagnosticInput) -> FlagEvaluation:
    ...
    if gate:
        return gate
    assert data.prior is not None
    current_margin = float(data.current.operating_income) / float(data.current.revenue)
    prior_margin = float(data.prior.operating_income) / float(data.prior.revenue)
    ...
    return _evaluated(data, flag, triggered, "RECENT_MARGIN_DECELERATION_THRESHOLD_MET" if triggered else "RECENT_MARGIN_DECELERATION_CONDITION_CLEAR", _evidence(
        current_revenue=data.current.revenue, prior_revenue=data.prior.revenue,
        current_operating_income=data.current.operating_income, prior_operating_income=data.prior.operating_income,
        current_operating_margin=current_margin, prior_operating_margin=prior_margin,
...
def _working_capital(data: DiagnosticInput) -> FlagEvaluation:
    flag = FLAG_NAMES[6]
    if data.canonical_fiscal_chain_consecutive is not None:
        data = DiagnosticInput(
            data.current, data.prior, data.canonical_fiscal_chain_consecutive
        )
...
def evaluate_diagnostic_flags(data: DiagnosticInput) -> tuple[FlagEvaluation, ...]:
    """Evaluate the seven independent V2 diagnostics in canonical order."""
