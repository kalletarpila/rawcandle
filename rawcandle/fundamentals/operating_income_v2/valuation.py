...
from .valuation_math import ANCHORS, piecewise_points

from .contract import OPERATING_YIELD_ANCHORS, VALUATION_MODEL_VERSION, model_fingerprint
...
MODEL_CONTRACT = {
    ...
    "full_requirements": [
        ...
        "observed_operating_income_fcf_and_common_earnings",
    ],
    "decision_precedence": [
...
MODEL_CONTRACT["operating_income_fallback"] = None
...
def _result(observation: ValuationObservation, **values: Any) -> ValuationResult:
    payload = {
        ...
        "ttm_operating_income": _number(observation.ttm_operating_income),
        ...
        "operating_income_yield": None,
        "operating_income_points": None,
        "fcf_yield": None,
        "fcf_points": None,
        "earnings_yield": None,
...
def calculate_valuation(
    ...
    operating_income = _number(observation.ttm_operating_income)
    fcf = _number(observation.ttm_free_cashflow)
    common = _number(observation.ttm_net_income_common)
    if operating_income is None:
        return _result(observation, valuation_status="VALUATION_NOT_READY", reason_code="TTM_OPERATING_INCOME_MISSING", **balance_values)
    if fcf is None:
        return _result(observation, valuation_status="VALUATION_NOT_READY", reason_code="TTM_FCF_MISSING", **balance_values)
    assert common is not None

    operating_income_yield = operating_income / enterprise_value
    fcf_yield = fcf / market_cap
    earnings_yield = common / market_cap
    operating_income_points = piecewise_points(operating_income_yield, OPERATING_YIELD_ANCHORS)
    ...
    return _result(
        ...
        operating_income_yield=operating_income_yield,
        operating_income_points=operating_income_points,
        fcf_yield=fcf_yield,
        fcf_points=fcf_points,
        earnings_yield=earnings_yield,
        earnings_points=earnings_points,
        total_valuation_score=operating_income_points + fcf_points + earnings_points,
        **balance_values,
    )
