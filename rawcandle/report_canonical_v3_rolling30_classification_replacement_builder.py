...
def _load_target_coverage(conn: sqlite3.Connection, run_row: sqlite3.Row) -> list[sqlite3.Row]:
    ...
    if not rows:
        raise ValueError(f"Missing eligible TICKER rolling30 coverage rows for run_id '{run_row['run_id']}'")
    return rows
...
def _context_row_for_classifier(
    *,
    ...
    output_row = {
        ...
        "fast_ema10_pullback_days": sum(1 for row in current_rows if int(row["fast_ema10_pullback_signal"] or 0) == 1),
        "conservative_ema20_pullback_days": sum(
            1 for row in current_rows if int(row["conservative_ema20_pullback_signal"] or 0) == 1
        ),
        ...
        "latest_bearish_relevance_class": (
            last_row["latest_bearish_relevance_class"] if "latest_bearish_relevance_class" in ticker_columns else None
        ),
        "latest_bullish_relevance_class": (
            last_row["latest_bullish_relevance_class"] if "latest_bullish_relevance_class" in ticker_columns else None
        ),
...
def _buy_classification_row(
    *,
    ...
    result = classify_rolling_30_buy_row(classifier_row)
    classification_state = _normalize_text(result.rolling_30_buy_state)
    ...
    return {
        ...
        "blocking_reason": _normalize_text(result.blocking_reason),
        "risk_reason": None,
        "next_action": None,
        "priority_score": None,
        "priority_label": None,
...
def _exit_classification_row(
    *,
    ...
    result = classify_rolling_30_exit_row(classifier_row)
    classification_state = _normalize_text(result.rolling_30_exit_state)
    decision_status = "OK" if classification_state else "MISSING"
    return {
        "run_id": str(run_row["run_id"]),
        "ecosystem_id": int(run_row["ecosystem_id"]),
...
def build_canonical_v3_rolling30_watchlist_classifications(
    db_path: str,
    run_id: str,
    ...
    try:
        ...
        if existing_count and not replace_existing:
            raise ValueError(
                ...
                f"run_id '{run_row['run_id']}' and classification_type IN {CLASSIFICATION_TYPES}"
            )
        ...
        classification_rows_inserted_by_type: Counter[str] = Counter()
        classification_state_counts_by_type: dict[str, Counter[str]] = {
            "rolling30_buy": Counter(),
            "rolling30_exit": Counter(),
        }
        decision_status_counts_by_type: dict[str, Counter[str]] = {
            "rolling30_buy": Counter(),
            "rolling30_exit": Counter(),
        }
        field_coverage_counts_by_type: dict[str, Counter[str]] = {
            "rolling30_buy": Counter(
                {
                    "primary_reason": 0,
                    "blocking_reason": 0,
        ...
        explicit_source_run_id = (
            f"V3_ROLLING30_CLASSIFICATION_FROM_LOWER_LEVEL_{str(run_row['signal_date']).replace('-', '_')}"
        )
        ...
        for coverage_row in coverage_rows:
            ...
            if not current_rows:
                warnings.append(f"Missing lower-level ticker history for rolling30 ticker '{ticker}'")
                continue
            ...
            subindustry_row = None if subindustry_name is None else group_rows_by_key.get(("subindustry", subindustry_name))
            ...
            for decision_row in (buy_row, exit_row):
                decision_rows.append(decision_row)
                classification_type = str(decision_row["classification_type"])
                classification_rows_inserted_by_type[classification_type] += 1
                classification_state_counts_by_type[classification_type][
                    str(decision_row["classification_state"])
                ] += 1
                decision_status_counts_by_type[classification_type][
                    str(decision_row["decision_status"])
                ] += 1
                for field_name in field_coverage_counts_by_type[classification_type]:
                    if decision_row[field_name] is not None:
                        field_coverage_counts_by_type[classification_type][field_name] += 1
        ...
        limitations = [
            "replaces only rolling30_buy and rolling30_exit",
            ...
            "daily_trigger remains transitional",
            "rolling2_sell_pressure and rolling5_pullback remain as-is",
        ...
        return {
            ...
            "taxonomy_version_code": str(run_row["version_code"]),
            "signal_date": str(run_row["signal_date"]),
            "classification_types": list(CLASSIFICATION_TYPES),
            "window_code": WINDOW_CODE,
            ...
            "classification_rows_inserted_by_type": dict(classification_rows_inserted_by_type),
            "classification_state_counts_by_type": {
                key: dict(counter) for key, counter in classification_state_counts_by_type.items()
            },
            "decision_status_counts_by_type": {
                key: dict(counter) for key, counter in decision_status_counts_by_type.items()
            },
            "field_coverage_counts_by_type": {
                key: dict(counter) for key, counter in field_coverage_counts_by_type.items()
            },
