...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--production", action="store_true", help="Run guarded rebuild from the active taxonomy after a matching copy test.")
    ...
    try:
        if args.production and args.apply:
            if not args.preview_payload or not args.preview_fingerprint or not args.test_run_id:
                raise ValueError("--production --apply requires --preview-payload, --preview-fingerprint and --test-run-id")
            result = run_production_apply(
                ...
                confirm_production=args.confirm_production == "CONFIRM_PRODUCTION_TAXONOMY",
