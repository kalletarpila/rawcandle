...
def _load_target_coverage(conn: sqlite3.Connection, run_row: sqlite3.Row) -> list[sqlite3.Row]:
    ...
    if not rows:
        raise ValueError(f"Missing eligible TICKER rolling5 coverage rows for run_id '{run_row['run_id']}'")
    return rows
...
def _classification_row(
    *,
    ...
    result = classify_rolling_5_pullback_row(classifier_row)
    classification_state = _normalize_text(result.rolling_5_pullback_state)
...
def build_canonical_v3_rolling5_pullback_classifications(
    db_path: str,
    run_id: str,
    ...
    try:
        ...
        explicit_source_run_id = f"V3_ROLLING5_CLASSIFICATION_FROM_LOWER_LEVEL_{str(run_row['signal_date']).replace('-', '_')}"

        for coverage_row in coverage_rows:
            ticker = str(coverage_row["entity_code"])
            current_rows = ticker_rows_by_ticker.get(ticker)
            if not current_rows:
                warnings.append(f"Missing lower-level ticker history for rolling5 ticker '{ticker}'")
                continue
        ...
        limitations = [
            "replaces only rolling5_pullback",
            ...
            "other classification types remain as-is",
