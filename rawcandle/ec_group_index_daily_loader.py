    ...
    *,
    ...
    if calc_version is not None:
        row_count = int(
            conn.execute(
                ...
                """,
                (signal_date, calc_version, taxonomy_version_code),
            ).fetchone()[0]
        )
        if row_count == 0:
            raise ValueError(
    ...
    if not versions:
        raise ValueError(
            ...
            f"index_date={signal_date!r}, taxonomy_version={taxonomy_version_code!r}"
        )
    if len(versions) > 1:
        raise ValueError(
...
def _source_scope_failed_summary(
    *,
    ...
    return {
        ...
        "loader_error": error,
        "group_index_loader_error": error,
...
def _source_group_key(row: sqlite3.Row) -> tuple[str | None, str | None, str | None, str | None, str | None]:
    return (
        str(row["index_date"]) if row["index_date"] is not None else None,
...
def _group_ref(row: sqlite3.Row) -> str:
    return (
        ...
        f"{row['index_date']}:{row['calc_version']}"
    )
...
def load_ec_group_index_daily_from_dc(
    ...
    try:
        ...
        source_validation = _source_validation_summary(
            rows,
            requested_taxonomy_version=taxonomy_version_code,
            requested_calc_version=selected_calc_version,
        )
        if (
            ...
            return {
                ...
                "loader_error": "Group index source rows failed taxonomy scope validation",
                "group_index_loader_error": "Group index source rows failed taxonomy scope validation",
                "ecosystem_code": ecosystem_code,
                "taxonomy_version_code": taxonomy_version_code,
                "signal_date": selected_signal_date,
                "calc_version": selected_calc_version,
        ...
        with target_conn:
            ...
            if failed_row_count > 0:
                ...
                return {
                    ...
                    "loader_error": "Group index rows could not be resolved to one EC group entity",
                    "group_index_loader_error": "Group index rows could not be resolved to one EC group entity",
                    "ecosystem_code": ecosystem_code,
                    "taxonomy_version_code": taxonomy_version_code,
                    "signal_date": selected_signal_date,
                    "calc_version": selected_calc_version,
            ...
            if (
                ...
                return {
                    ...
                    "loader_error": "Group index rows produced duplicate or null EC target keys",
                    "group_index_loader_error": "Group index rows produced duplicate or null EC target keys",
                    "ecosystem_code": ecosystem_code,
                    "taxonomy_version_code": taxonomy_version_code,
                    "signal_date": selected_signal_date,
                    "calc_version": selected_calc_version,
            ...
            try:
                ...
                return {
                    ...
                    "group_index_loader_error": str(exc),
                    "ecosystem_code": ecosystem_code,
                    "taxonomy_version_code": taxonomy_version_code,
                    "signal_date": selected_signal_date,
                    "calc_version": selected_calc_version,
