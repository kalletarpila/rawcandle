...
DESTINATION_TABLE = "dc_dashboard_action_summary_daily"
CALC_VERSION = "DATACENTER_DASHBOARD_ACTION_SUMMARY_V1"


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Write Datacenter action summary rows into analysis.db."
    )
...
def _resolve_run_id(signal_date: str, explicit_run_id: str | None) -> str:
    ...
    return f"DC_DASH_ACTION_SUMMARY_{signal_date}_{_utc_now_text()}"
...
def _summarize_actions(rows: list[sqlite3.Row]) -> tuple[int, list[tuple[str, int]]]:
    counts: dict[str, int] = {}
    actionable_rows = 0
    for row in rows:
    ...
    return actionable_rows, sorted(counts.items(), key=lambda item: item[0])
...
def _existing_keys(
    ...
    *,
    ...
    return {
        (str(row["signal_date"]), str(row["taxonomy_version"]), str(row["action"])) for row in rows
    }


def _emit_summary(name: str, value: object) -> None:
    print(f"SUMMARY datacenter_dashboard_action_summary_write.{name}={value}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with connector(args.analysis_db) as conn:
            ...
            actionable_rows, action_counts = _summarize_actions(source_rows)
            ...
            row_specs = [
                (
                    (signal_date, taxonomy_version, action),
            ...
            if args.mode == "insert-missing":
                ...
                if not args.dry_run and row_specs:
                    conn.executemany(
                        ...
                        ON CONFLICT(signal_date, taxonomy_version, action) DO UPDATE SET
            ...
            _emit_summary("actionable_rows", actionable_rows)
            _emit_summary("distinct_actions", len(action_counts))
