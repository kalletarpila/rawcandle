...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render canonical rolling30 Markdown report from existing canonical V2 tables."
    )
...
def _write_output(path_str: str, markdown: str) -> None:
    path = Path(path_str)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        with _connect_read_only(str(args.db)) as conn:
            formatter_data = load_rolling30_canonical_formatter_data_v2(
                conn,
    ...
    if args.output:
        _write_output(str(args.output), markdown)
    else:
        print(markdown, end="")
    return 0
