...
The repository still contains a substantial Canonical Report V2 stack around `dc_report_*_v2`: schema migrations, a migration applier, analysis builders/writers, formatter loaders, parity audit code, retired dev_tools CLI stubs, production/publish documentation, and extensive non-CLI tests. It does not appear wired into the current scheduler path. The original audit found that `analysis/database_manager.py` applied the Canonical V2 migrations during general analysis DB initialization; Phase B has now neutralized that hook. Docs still record an accepted manual production publish baseline for `2026-05-29`.

The safe next step is not DB cleanup. Remove remaining V2 core code/tests/docs in Phase C before any `dc_report_*_v2` table cleanup.
...
| `rawcandle/report_canonical_v2_migration.py` | Migration applier | SAFE_REMOVE_LATER after Phase B | Applies migrations `004`-`014` and idempotent column patches for V2 readmodel tables. No longer active via `DatabaseManager` or V2 dev_tools entrypoints after Phase B. | Remove in Phase C/E only after remaining direct test/core references are handled and migration strategy is explicit. |
| `analysis/datacenter_indices/report_canonical_v2_orchestrator.py` | V2 builder orchestrator | AMBIGUOUS | Writes `dc_report_run_v2` and invokes builders/classification writers for V2 rows. Not scheduler-wired, but central to manual Canonical V2 build flow. | If V2 is retired, remove after migration hook and CLIs are neutralized. |
| `analysis/datacenter_indices/report_canonical_v2_group_context_builder.py` | V2 context builder | AMBIGUOUS | Reads current `dc_group_swing_signal_daily` and `dc_group_synthetic_ohlc_daily`, writes `dc_report_context_group_v2`. Mixed current-source reads with obsolete readmodel writes. | Split preserve/remove boundary carefully; preserve source facts, remove only V2 writer if retiring V2. |
| `analysis/datacenter_indices/report_canonical_v2_daily_context_builder.py` | V2 context builder | AMBIGUOUS | Reads `dc_ticker_swing_signal_daily` and `dc_report_context_group_v2`, writes `dc_report_context_daily_v2`. | Remove only after V2 workflow is retired; preserve `dc_ticker_swing_signal_daily`. |
| `analysis/datacenter_indices/report_canonical_v2_window_context_builder.py` | V2 context builder | AMBIGUOUS | Reads current `dc_*` facts and `dc_report_context_group_v2`, writes `dc_report_context_window_v2`. | Remove only after V2 workflow is retired. |
| `analysis/datacenter_indices/report_canonical_v2_*_classification_writer.py` | V2 classification writers | SAFE_REMOVE_LATER after decision | Write `dc_report_classification_v2` from V2 context tables. They belong to V2 readmodel, not current `dc_*` source facts. | Remove in Phase C after active hooks and manual V2 decision are resolved. |
| `analysis/datacenter_indices/report_canonical_v2_*_formatter_loader.py` | V2 formatter/query/readmodel loaders | SAFE_REMOVE_LATER after decision | Read `dc_report_*_v2` tables to render Canonical V2 outputs. | Remove with dev_tools output CLIs if V2 outputs are retired. |
| `analysis/datacenter_indices/report_canonical_v2_parity_audit.py` | V2 parity/audit code | AMBIGUOUS | Part of accepted manual Canonical V2 publish workflow; reads `dc_report_classification_v2` and compares with current/source behavior. | Decide whether Canonical V2 manual publish remains supported. |
| `dev_tools/run_report_canonical_v2_output.py` | Retired manual render CLI stub | SAFE_REMOVE_LATER after Phase B | Phase B replaced executable behavior with a retired entrypoint that exits non-zero and does not open DBs or import V2 formatter modules. | Delete with remaining V2 dev_tools stubs in Phase C. |
| `dev_tools/run_report_canonical_v2_publish_outputs.py` | Retired manual publish CLI stub | SAFE_REMOVE_LATER after Phase B | Phase B replaced executable behavior with a retired entrypoint that exits non-zero and does not publish outputs. | Delete with remaining V2 dev_tools stubs in Phase C. |
| `dev_tools/run_report_canonical_v2_daily_markdown_smoke.py` and `run_report_canonical_v2_all_outputs_smoke.py` | Retired build/smoke CLI stubs | SAFE_REMOVE_LATER after Phase B | Phase B removed build/smoke behavior from these entrypoints; they no longer apply V2 migrations or write DB/output state. | Delete with remaining V2 dev_tools stubs in Phase C. |
| `dev_tools/run_report_canonical_v2_*_{markdown,csv}.py` | Retired manual output CLI stubs | SAFE_REMOVE_LATER after Phase B | Phase B replaced render behavior with retired entrypoints that exit non-zero. | Delete with formatter loaders in Phase C. |
| `dev_tools/run_report_canonical_v2_parity_audit.py` | Retired manual parity CLI stub | SAFE_REMOVE_LATER after Phase B | Phase B replaced parity behavior with a retired entrypoint that exits non-zero and does not import V2 parity code. | Delete with remaining V2 dev_tools stubs in Phase C. |
| `tests/test_report_canonical_v2_*.py` | Test suite | SAFE_REMOVE_LATER after Phase B | Direct V2 CLI tests were replaced with retired-entrypoint coverage; schema/core/builder/formatter tests remain for Phase C. | Remove/update remaining V2 tests after core code is retired. |
