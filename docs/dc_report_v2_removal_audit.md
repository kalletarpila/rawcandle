...
|---|---|---|---|---|
| `analysis/database_manager.py` | DB initialization hook | AMBIGUOUS | General analysis DB initialization imports and calls `apply_report_canonical_v2_migration(conn)`, so V2 schema is still actively installed for new analysis DBs. | Decide whether this migration hook must be removed/neutralized before deleting V2 schema/code. |
| `rawcandle/report_canonical_v2_migration.py` | Migration applier | AMBIGUOUS | Applies migrations `004`-`014` and idempotent column patches for V2 readmodel tables. Active via `DatabaseManager` and dev_tools smoke flows. | Do not remove until Canonical V2 support decision is explicit. |
...
| `dev_tools/run_report_canonical_v2_output.py` | Manual read-only render CLI | AMBIGUOUS | Renders one Canonical V2 output from existing `dc_report_*_v2` rows; production docs say manual use is safe. | Do not remove until manual Canonical V2 output policy is revoked. |
| `dev_tools/run_report_canonical_v2_publish_outputs.py` | Manual publish CLI | AMBIGUOUS | Publishes all eight Canonical V2 outputs from existing V2 rows; docs mark workflow accepted. | Do not remove until canonical delivery policy is retired or replaced. |
| `dev_tools/run_report_canonical_v2_daily_markdown_smoke.py` and `run_report_canonical_v2_all_outputs_smoke.py` | Build/smoke CLIs | AMBIGUOUS | Can apply V2 migration and run V2 build/smoke flows, potentially DB-writing against supplied DBs. | Neutralize before removing core V2 code if V2 is retired. |
| `dev_tools/run_report_canonical_v2_*_{markdown,csv}.py` | Manual output CLIs | AMBIGUOUS | Render specific V2 outputs from V2 tables. | Remove with formatter loaders after manual V2 support decision. |
| `dev_tools/run_report_canonical_v2_parity_audit.py` | Manual parity CLI | AMBIGUOUS | Supports manual V2 validation. | Remove only after V2 parity workflow is retired. |
| `tests/test_report_canonical_v2_*.py` | Test suite | SAFE_REMOVE_LATER after decision | Tests schema, builders, formatters, CLIs, smoke, and publish behavior for V2. | Remove/update after code path is retired. |
