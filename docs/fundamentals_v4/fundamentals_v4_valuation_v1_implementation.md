...
Phase 3D adds the exact-path production CLI and the normal Score -> Lifecycle -> Valuation orchestration hook. The CLI is dry-run by default, requires the locked fingerprint and full-universe scope, and requires separate apply and production-confirmation flags. Operational Valuation refresh uses its own analysis transaction and returns zero writes when logical rows are unchanged; a failed refresh preserves the prior valuation dataset and fails the parent Fundamentals run explicitly.

Phase 3D was deployed on 2026-09-01. See `fundamentals_v4_valuation_v1_phase3d_deployment.md` for backups, commands, migration counts, production fingerprints, distributions, reader checks, and regressions.
