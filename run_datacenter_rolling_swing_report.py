...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render a read-only datacenter rolling swing report for the last N valid trading days."
    )
    parser.add_argument("--analysis-db", type=Path, required=True, help="Path to analysis.db")
    parser.add_argument("--end-date", type=str, required=True, help="Rolling window end date (YYYY-MM-DD)")
...
def _default_output_path(*, window_size: int, end_date: str, suffix: str) -> Path:
    return Path.cwd() / f"datacenter_rolling_{window_size}_{end_date}_full.{suffix}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_md_seed = (
        ...
        else _default_output_path(window_size=args.window_size, end_date=args.end_date, suffix="md")
    )
    output_csv_seed = (
        args.output_csv
        if args.output_csv is not None
        else _default_output_path(window_size=args.window_size, end_date=args.end_date, suffix="csv")
    )
    ...
    print("SUMMARY report_type=rolling")
