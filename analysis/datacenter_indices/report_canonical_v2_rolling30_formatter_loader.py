...
def _load_group_rows(
    ...
    *,
    ...
    where_clauses = ["signal_date = ?", "taxonomy_version = ?", "horizon = 'rolling30'"]
    params: list[object] = [signal_date, taxonomy_version]
    if market is None:
        where_clauses.append("market IS NULL")
    else:
...
def _load_classification_rows(
    ...
    *,
    ...
    where_clauses = [
        ...
        "taxonomy_version = ?",
        "horizon = 'rolling30'",
        "classification_type IN ('rolling30_buy', 'rolling30_exit')",
    ]
    params: list[object] = [signal_date, taxonomy_version]
    if market is None:
        where_clauses.append("market IS NULL")
    ...
    rows = conn.execute(
        ...
        WHERE {' AND '.join(where_clauses)}
        ORDER BY classification_type ASC, ticker ASC
        """,
        tuple(params),
    ).fetchall()
    return [_row_to_dict(row) for row in rows]
...
def _build_rolling30_buy_rows(
    *,
    ...
    for classification_row in classification_rows:
        if classification_row.get("classification_type") != "rolling30_buy":
            continue
...
def _build_rolling30_exit_rows(
    *,
    window_rows: list[dict[str, object]],
    classification_rows: list[dict[str, object]],
) -> list[dict[str, object]]:
    ...
    output_rows: list[dict[str, object]] = []
    for classification_row in classification_rows:
        if classification_row.get("classification_type") != "rolling30_exit":
            continue
        ticker = str(classification_row.get("ticker") or "")
        window_row = window_by_ticker.get(ticker, {})
        output_rows.append(
            {
                **classification_row,
                ...
                "window_watchlist_status": window_row.get("window_watchlist_status"),
                "exit_risk_days": window_row.get("exit_risk_days"),
                "high_exit_risk_days": window_row.get("high_exit_risk_days"),
                "medium_exit_risk_days": window_row.get("medium_exit_risk_days"),
                "exit_risk_severity": window_row.get("exit_risk_severity"),
                "latest_exit_reason": window_row.get("latest_exit_reason"),
                "trend_state": window_row.get("trend_state"),
                "latest_structure_label": window_row.get("latest_structure_label"),
...
def _build_section_counts(
    *,
    group_rows: list[dict[str, object]],
    window_rows: list[dict[str, object]],
    rolling30_buy_rows: list[dict[str, object]],
    rolling30_exit_rows: list[dict[str, object]],
    ...
    buy_state_counts: dict[str, int] = {}
    for row in rolling30_buy_rows:
        state = str(row.get("classification_state") or "")
        buy_state_counts[state] = buy_state_counts.get(state, 0) + 1

    exit_state_counts: dict[str, int] = {}
    for row in rolling30_exit_rows:
        state = str(row.get("classification_state") or "")
        exit_state_counts[state] = exit_state_counts.get(state, 0) + 1
    ...
    return {
        ...
        "rolling30_buy_classification_row_count": len(rolling30_buy_rows),
        "rolling30_exit_classification_row_count": len(rolling30_exit_rows),
        ...
        "rolling30_buy_classification_state_counts": dict(sorted(buy_state_counts.items())),
        "rolling30_exit_classification_state_counts": dict(sorted(exit_state_counts.items())),
...
def load_rolling30_canonical_formatter_data_v2(
    conn: sqlite3.Connection,
    *,
    signal_date: str,
    taxonomy_version: str,
    ...
    rolling30_buy_rows = _build_rolling30_buy_rows(
        window_rows=window_rows,
        classification_rows=classification_rows,
    )
    rolling30_exit_rows = _build_rolling30_exit_rows(
        window_rows=window_rows,
        classification_rows=classification_rows,
    )
    ...
    section_counts = _build_section_counts(
        group_rows=group_rows,
        window_rows=window_rows,
        rolling30_buy_rows=rolling30_buy_rows,
        rolling30_exit_rows=rolling30_exit_rows,
...
def build_markdown_rolling30_canonical_v2_report(formatter_data: dict[str, object]) -> str:
    ...
    lines: list[str] = [
        "# Datacenter Rolling30 Canonical V2 Report",
        "",
        "## 1. Title / metadata",
        ...
        f"- rolling30_buy_classification_count: {_markdown_value(section_counts.get('rolling30_buy_classification_row_count'))}",
        f"- rolling30_exit_classification_count: {_markdown_value(section_counts.get('rolling30_exit_classification_row_count'))}",
    ...
    buy_state_counts = dict(section_counts.get("rolling30_buy_classification_state_counts") or {})
    if buy_state_counts:
        for state, count in sorted(buy_state_counts.items()):
            lines.append(f"- {state}: {count}")
    else:
        lines.append("- none")

    lines.extend(["", "### Rolling30 exit classification state counts"])
    exit_state_counts = dict(section_counts.get("rolling30_exit_classification_state_counts") or {})
    if exit_state_counts:
        for state, count in sorted(exit_state_counts.items()):
            lines.append(f"- {state}: {count}")
    else:
        lines.append("- none")
    ...
    lines.append("## 3. Rolling30 buy rows")
    ...
    lines.append("## 4. Rolling30 exit rows")
    _append_markdown_table(
        lines,
        headers=[
            "ticker",
    ...
    lines.append("## 6. Repeated breakout rows")
    _append_markdown_table(
        lines,
        headers=[
            "ticker",
    ...
    lines.append("## 7. Repeated pullback rows")
    _append_markdown_table(
        lines,
        headers=[
            "ticker",
    ...
    lines.append("## 8. Repeated exit-risk rows")
    _append_markdown_table(
        lines,
        headers=[
            "ticker",
    ...
    lines.append("## 9. Taxonomy listing preview")
    _append_markdown_table(
        lines,
        headers=[
            "row_type",
    ...
    lines.append("## 10. Deferred sections")
    labels = {
