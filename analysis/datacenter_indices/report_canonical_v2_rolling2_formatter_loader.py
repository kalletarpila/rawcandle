...
def _load_group_rows(
    ...
    *,
    ...
    where_clauses = ["signal_date = ?", "taxonomy_version = ?", "horizon = 'rolling2'"]
    params: list[object] = [signal_date, taxonomy_version]
    if market is None:
        where_clauses.append("market IS NULL")
    else:
...
def _build_rolling2_sell_pressure_rows(
    *,
        ...
        output_rows.append(
            {
                ...
                "return_2d": layer_row.get("return_2d"),
                "return_5d": layer_row.get("return_5d"),
                "return_30d": layer_row.get("return_30d"),
            ...
            output_rows.append(
                {
                    ...
                    "return_2d": subindustry_row.get("return_2d"),
                    "return_5d": subindustry_row.get("return_5d"),
                    "return_30d": subindustry_row.get("return_30d"),
                ...
                output_rows.append(
                    {
                        ...
                        "exit_risk_days": window_row.get("exit_risk_days"),
                        "distance_to_ema20_pct": window_row.get("distance_to_ema20_pct"),
                        "trend_state": window_row.get("trend_state"),
                        "latest_structure_label": window_row.get("latest_structure_label"),
                        "latest_bos_event_type": window_row.get("latest_bos_event_type"),
                        "latest_bos_freshness": window_row.get("latest_bos_freshness"),
...
def _build_section_counts(
    *,
    group_rows: list[dict[str, object]],
    window_rows: list[dict[str, object]],
    rolling2_sell_pressure_rows: list[dict[str, object]],
    ...
    for row in rolling2_sell_pressure_rows:
    ...
    return {
        ...
        "rolling2_classification_row_count": len(rolling2_sell_pressure_rows),
        ...
        "rolling2_classification_state_counts": dict(sorted(classification_state_counts.items())),
...
def load_rolling2_canonical_formatter_data_v2(
    conn: sqlite3.Connection,
    *,
    signal_date: str,
    taxonomy_version: str,
    ...
    rolling2_sell_pressure_rows = _build_rolling2_sell_pressure_rows(
    ...
    section_counts = _build_section_counts(
        group_rows=group_rows,
        window_rows=window_rows,
        rolling2_sell_pressure_rows=rolling2_sell_pressure_rows,
...
def build_markdown_rolling2_canonical_v2_report(formatter_data: dict[str, object]) -> str:
    metadata = dict(formatter_data.get("metadata") or {})
    run = formatter_data.get("run")
    window_rows = list(formatter_data.get("window_rows") or [])
    rolling2_sell_pressure_rows = list(formatter_data.get("rolling2_sell_pressure_rows") or [])
    ...
    lines: list[str] = [
        "# Datacenter Rolling2 Canonical V2 Report",
        "",
        "## 1. Title / metadata",
        ...
        f"- rolling2_classification_count: {_markdown_value(section_counts.get('rolling2_classification_row_count'))}",
    ...
    classification_state_counts = dict(section_counts.get("rolling2_classification_state_counts") or {})
    ...
    lines.append("## 3. Rolling2 sell pressure rows")
...
def build_csv_rolling2_canonical_v2_report(formatter_data: dict[str, object]) -> str:
    metadata = dict(formatter_data.get("metadata") or {})
    run = formatter_data.get("run")
    window_rows = list(formatter_data.get("window_rows") or [])
    rolling2_sell_pressure_rows = list(formatter_data.get("rolling2_sell_pressure_rows") or [])
    ...
    summary_rows = [
        ...
        ("rolling2_classification_count", section_counts.get("rolling2_classification_row_count")),
    ...
    for key, value in sorted(dict(section_counts.get("rolling2_classification_state_counts") or {}).items()):
        row = _empty_row("rolling2_classification_state_counts")
        row["key"] = str(key)
        row["value"] = _csv_value(value)
        rows.append(row)
    ...
    for source_row in rolling2_sell_pressure_rows:
        row = _empty_row("rolling2_sell_pressure_rows")
