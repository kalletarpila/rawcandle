...
class TestCSVExport(unittest.TestCase):
    ...
    def test_csv_generation_with_valid_data(self):
        ...
        self.assertTrue(
            os.path.exists(file_path), f"CSV-tiedostoa ei löytynyt: {file_path}"
        )
        ...
        self.assertEqual(
            actual_cols,
            expected_cols,
            f"CSV:ssä pitää olla {expected_cols} saraketta, nyt {actual_cols}",
        )
    ...
    def test_data_formatting(self):
        ...
        self.assertTrue(
            all(
                formatted_data.dtypes.apply(
                    lambda x: x in ["float64", "int64", "object"]
                )
            )
        )
...
def run_tests():
    ...
    print("\n📊 Testien tulokset:")
    print(
        f"✅ Onnistuneet: {result.testsRun - len(result.failures) - len(result.errors)}"
    )
